...
| `review_cadence.py` | **complete (M0-T042).** The D-010 0A.3 cadence policy: a deterministic classifier that reviews meaningful checkpoints (unit-complete, before-merge, correction, security/architectural change, evidence conflict, unclassifiable next action) and refuses to spend a review merely to restate a passing formatter/linter/unit-test. No model call, no process, no state mutation. |
| `review_packet.py` | **complete (M0-T042).** Two fail-closed admission gates before any review process: the 0A.4 token budget (target 32k, ordinary ceiling 64k, relative ceiling 20% of the reported window, effective ceiling the lower of the two, with recorded estimate/bytes/sources/omissions/truncations and split-summarize guidance on overflow) and the AD-083 prohibited-content guard (reject-or-strip of whole transcript, full directive registry, all historical reports, unrelated task packets, the whole repository, all logs, and full code-graph dumps). |
| `ephemeral_review.py` | **complete (M0-T042).** The operational fresh-ephemeral Codex review loop: guard then budget then a fresh read-only process then a redacted, digest-sealed durable record (decision, evidence refs, model identity, usage telemetry, digest — 0A.1 item 7). Proves per-review independence (AD-027), records sources Codex reopened beyond the packet (AD-087/0A.6), and builds — never activates — the writable worker-fallback exception record (AD-088/0A.5). |
