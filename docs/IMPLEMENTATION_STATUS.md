...
Authoritative detail lives in `project-control/` (run `python tools/project_control.py status`). This file is the human-readable summary. Updated: 2026-07-15 (end of session 3).
...
| Task | State |
|---|---|
| M0-T009 Canonical contracts v1 | Producer complete on branch `task/M0-T009-contracts-v1` (522d3b2, pushed). G2 evidence captured (exit 0 both checks; `project-control/reports/M0-T009-G2-evidence.md`). Pending: G3 (code-reviewer), merge, G4, G5 (security-reviewer). |

### In progress
| Task | State |
|---|---|
| M0-T005 Secrets policy + secret-scan CI | claimed; producer was running in worktree `agent-a885db23d6225afb2` at session end — see SESSION_HANDOFF for recovery steps. |

### Ready / next
- M0-T011 / ADR-004 — drop Vercel, serve Next.js from Render (owner decision 2026-07-14). Unblocked by M0-T006 acceptance. Fold in G3 residuals R1 (ADR-001:36 service-role line) and the `"off"`-quoting recommendation.
- Follow-up tasks to create before M0 exit: D5 production deploy workflow implementation; frontend deploy gating config (vercel.json — moot if ADR-004 drops Vercel; decide inside M0-T011).
- M0-T004 G5 follow-ups (medium/low): SHA-pin actions before any CI secret; Dependabot/audit; Python lockfile; remove/restrict generate-lockfile.yml.
- M1-T001 PLUTO/MapPLUTO research — packet contracted (backlog); first of the M1 source-family fan-out.

### Blocked (human actions — see docs/HUMAN_ACTIONS_REQUIRED.md)
- B-001 SUPABASE_ACCESS_TOKEN → M0-T007/T008
- B-002 RENDER_API_KEY → service creation (temporary chat key not available to this session; owner must supply a fresh key or use the dashboard Blueprint flow)
- B-003 Vercel → superseded pending ADR-004; close when ADR-004 lands
- B-004 GEOCLIENT_SUBSCRIPTION_KEY → live fixtures
- B-005 3D/UI expansion pack incomplete → M0-T010
...
- Repo: private `martin10101/nyc-buildability`; monorepo live (Next.js 15 placeholder, FastAPI placeholder, contracts v1 schemas, migrations placeholder).
- CI: 4 jobs green on main (run 29457969862) including the new render.yaml Blueprint YAML validation step.
