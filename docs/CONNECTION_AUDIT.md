...
| Render | Not connected / Human action required | No CLI, no MCP, no API key (blocker B-002); `render.yaml` Blueprint can be authored without it |
| Vercel | Not connected / Human action required | No CLI/token (blocker B-003); not needed until frontend exists |
