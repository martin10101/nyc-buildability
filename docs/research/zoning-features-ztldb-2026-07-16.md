...
- Metadata fetch: `https://data.cityofnewyork.us/api/views/fdkv-4t4z.json` (Z2, retrieved 2026-07-16).
  ...
  - Description (verbatim): *"The Zoning Tax Lot Database is a comma-separated values (CSV) file that contains up-to-date zoning by parcel. The Database includes the zoning designations and zoning map associated with a specific tax block and lot. The Database is updated on a monthly basis to reflect rezoning and corrections to the file. All previously released versions of this data are available on the [DCP Website: BYTES of the BIG APPLE](https://www.nyc.gov/content/planning/pages/resources/datasets/zoning-map-index)."* **[CORRECTED AT G1 — C1]:** the final sentence (omitted in the first capture) is the officially-linked ZTLDB archive pointer (HTTP 403 to non-browser clients, verified 2026-07-16). The link target (`.../datasets/zoning-map-index`) is odd for a ZTLDB archive — recorded as-is, intent not guessed.
...
- Metadata fetch: `https://data.cityofnewyork.us/api/views/mm69-vrje.json` (Z6, retrieved 2026-07-16, description re-fetched verbatim):
  ...
  - Raw timestamps: createdAt = rowsUpdatedAt = publicationDate = **1359482950 = 2013-01-29T18:09:10Z** — frozen; blobby datasets do not update row timestamps. **[CORRECTED AT G1 — C2]:** `viewLastModified` = 1779809507 = **2026-05-26T15:31:47Z** (live 2026-07-16) is NOT frozen — it matches data.gov "Dataset Last Updated: May 26, 2026" and is the day after the last Monday of May 2026. Use `viewLastModified` as the primary blob-change polling signal; it implies the blob likely updated ~2026-05-26 while the description string still says 202604 (strengthens the stale-description hypothesis, §5.2/OQ-4).
...
| Special purpose districts | `nysp/FeatureServer` (layer 0 "nysp") | YES | polygon | 102718 / 2263 | **92** | **95** (G1, C3) | 1782912605345 ≈ 2026-07-01T13:30:05Z |
| Special purpose subdistricts | `nysp_sd/FeatureServer` (layer 0 "nysp_sd") | YES | polygon | 102718 / 2263 | **317** | **336** (G1, C3) | 1782912514806 ≈ 2026-07-01T13:28:34Z |
| Limited height districts | `nylh/FeatureServer` (layer 0 "nylh") | YES | polygon | 102718 / 2263 | **14** | **14** (G1, C3) | 1782912700015 ≈ 2026-07-01T13:31:40Z |
| Zoning map amendments | `nyzma/FeatureServer` (layer 0 "nyzma") | YES | polygon | 102718 / 2263 | 1292 | **1,414** (G1, C3) | 1782912214592 ≈ 2026-07-01T13:23:34Z |

**[CORRECTED AT G1 — C3]:** live counts for the four remaining layers (2026-07-16, G1 review): nysp **95**, nysp_sd **336**, nylh **14**, nyzma **1,414**. HAZARD: counts EXCEED `maxRecordCount` on nysp (95>92), nysp_sd (336>317), and nyzma (1414>1292), and nylh sits exactly at its cap (14=14) — an unpaged request silently truncates. **Paging is mandatory on every layer, not only nyzd/nyco.**

- All six layers were data-last-edited on **2026-07-01 between ~13:23 and ~13:34 UTC** — a coordinated refresh consistent with the monthly release model. **[RESOLVED AT G1 — C6]:** all eight epoch→UTC conversions were machine-re-verified correct to the second by the G1 reviewer (OQ-9 resolved).
...
- `https://www.nyc.gov/content/planning/pages/resources/datasets/zoning-taxlot-database` (ZTLDB page) — search-evidenced only **[NEEDS G1 RE-VERIFICATION]** (Z14; the G1 reviewer did not re-verify this URL either — marker stands).
- **[ADDED AT G1 — C1]:** the `fdkv-4t4z` description officially links the ZTLDB archive as `https://www.nyc.gov/content/planning/pages/resources/datasets/zoning-map-index` (HTTP 403 to non-browser clients, verified 2026-07-16; odd link target recorded as-is, intent not guessed).
...
- **nyzma:** OBJECTID, **EFFECTIVE** (esriFieldTypeDate), **STATUS** (String 15), **ULURPNO** (String 50), **LUCATS** (String 10), **PROJECT_NAME** (String 100), Shape__Area/Length. Description (verbatim, **[CORRECTED AT G1 — C5]** — source typo preserved and full third sentence restored): *"Outlines or for all zoning changes adopted and proposed since January 1, 2002. Includes outlines for current Certified rezonings. Selected city-initiated text amendments to the Zoning Resolution since 2002 that have discrete geographical boundaries may be included."* — directly relevant to the PRD's pending-land-use-action flags, including city-initiated text amendments with mapped boundaries (value domains: OQ-7).
...
- ArcGIS: all six layers data-last-edited **2026-07-01** (Z8/Z9). Socrata `mm69-vrje` description: **"Current version: 202604"** (Z6); data.gov last checked 2026-05-26 still 202604 (Z13). The Socrata blob channel appears 2–3 monthly cycles behind the ArcGIS channel as of 2026-07-16 — OR the description string is not maintained per release while the blob silently updates. **[CORRECTED AT G1 — C2]:** `viewLastModified` = 2026-05-26T15:31:47Z is live and moving (unlike the frozen 2013 blobby triple), implying the blob likely DID update ~2026-05-26 while the description string lags — the stale-description hypothesis is now the stronger one. Poll `viewLastModified` (primary) plus the description string and blob hash across the next monthly boundary. OQ-4.
...
| Socrata `mm69-vrje` | "Current version: YYYYMM" inside free-text description; blob row-timestamps frozen 2013; **`viewLastModified` live (2026-05-26, C2)** | description string weak/stale; `viewLastModified` is the primary blob-change polling signal (OQ-4) |
...
| OQ-9 | Eight ArcGIS epoch-ms→UTC conversions were done manually (deterministic arithmetic anchored to the machine-verified `1775414816 = 2026-04-05T18:46:56Z`) because the Bash tool was permission-denied mid-task | **RESOLVED AT G1 (C6)** — all eight conversions machine-verified correct to the second by the G1 reviewer (G1 report §1.2) |
| OQ-10 | Exact glyphs of the nyzd accuracy statement ("+/- 20 feet" rendered ambiguously in PDF extraction) | **RESOLVED AT G1 (C7)** — ±20 ft semantics confirmed by independent direct read; glyph ambiguity is an extraction artifact |
