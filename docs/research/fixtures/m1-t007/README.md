...
## bf97-mjsy — Construction-Related Incidents (C1v2 reproducibility evidence, added 2026-07-17)

**Classification: SECONDARY/FUTURE SOURCE — NOT part of the DOB NOW connector family.** Its documented system of record is the DOB Incident Database (dataset description verbatim, in the metadata extract), it carries no `job_filing_number`/`work_permit` keys, and it is dispositioned in findings §2.1 as a documented secondary/future source assigned to the DOB-wide/legacy research task (M1-T008 scope) feeding a later M2 property-history/risk fact stage. These fixtures exist ONLY to make the C1v2 disposition reproducible; they are not connector inputs. Captured by the orchestrator (evidence-capture division of labor; producer authored the disposition, harness isolation prevented its worktree writes). All fetched tokenless with a browser User-Agent via Python urllib.

- `bf97-mjsy_views_metadata_extract.json` — **EXTRACT** of `https://data.cityofnewyork.us/api/views/bf97-mjsy.json` (HTTP 200, retrieved 2026-07-17T09:01:33Z; raw response sha256 + byte size recorded inside the file). Kept: id, name, attribution, assetType, newBackend, provenance, createdAt, rowsUpdatedAt, description, custom_fields, attachments, full columns array reduced to fieldName/name/dataTypeName (20 columns). Fixture file sha256 `f488320cc11484810f92aff7339cdb605b8c936e4017d7aa34a60a38cb893feb`, 3,321 bytes.
- `query_bf97-mjsy_count.json` — **VERBATIM** `https://data.cityofnewyork.us/resource/bf97-mjsy.json?$select=count(*)` (HTTP 200, 2026-07-17T09:01:34Z) → `[{"count":"1326"}]`. sha256 `82f5f3927f34295d6307e200c0bdccc52e8e5af7ee91efa332c16e55ad2e2fc1`, 19 bytes. Point-in-time observation; drift expected.
- `query_bf97-mjsy_minmax_incident_date.json` — **VERBATIM** `https://data.cityofnewyork.us/resource/bf97-mjsy.json?$select=min(incident_date_mm_dd_yyyy)%20as%20min_d,max(incident_date_mm_dd_yyyy)%20as%20max_d` (HTTP 200, 2026-07-17T09:01:34Z) → min 2024-01-03, max 2026-07-14. sha256 `1abdc410e4b58f97a448432c8d27d5aef533bd72a1e6ff9ec1c5138537a860b3`, 72 bytes.
- `sample_bf97-mjsy.json` — **VERBATIM** `https://data.cityofnewyork.us/resource/bf97-mjsy.json?$limit=2&$order=incident_date_mm_dd_yyyy%20DESC` (HTTP 200, 2026-07-17T09:01:34Z), 2 rows. sha256 `ca5b9e56c4c2782a95f816ba3350b01af3cb2ba9d524a0b425f69e158cb0d216`, 1,100 bytes. NOTE: rows this recent may drop out of the top-2 as new incidents arrive; the stable reproducibility check is the row *shape* and key formats, not the specific records.
