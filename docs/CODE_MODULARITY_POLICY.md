...
- **Python API (services/api)**: `routes/*.py` (wiring only) → `services/*.py`
  (domain) → `repositories/*.py` (storage) → `serializers/*.py` (serialization;
  never a `schemas/` directory — that segment is excluded from measurement as
  generated-artifact space, §9, and handwritten code must stay measurable).
...
- can only be regenerated by `--regenerate-baseline --approval-id <id>` backed
  by an unexpired, SINGLE-USE `baseline-regeneration` approval bound to the one
  version it produces (`for_version`), and a regeneration **never erases live
  debt**: an entry whose file still exists at
  or above the warning threshold is carried forward at the smaller of its
  recorded and current size. Regeneration is also the ONE reviewed absorption
  path: a currently-failing new oversized file admitted by an approved
  regeneration becomes visible legacy debt in this file's diff - that is a
  reviewed act, never a silent one.
...
the check closed. Narrow and temporary are MACHINE-ENFORCED: `expires` may sit
at most 90 days ahead (`EXCEPTION_HORIZON_DAYS`), and a `max_lines` ceiling
grossly above the file's current size fails as `exception_too_broad` - an
exception permits at most one growth step. Renewing one requires fresh review
evidence, not editing the date.
...
- **SLOC** = physical lines that are non-blank and not comment-only
  (`#` for Python; `//` and `/* … */` for TS/TSX, span-aware: code sharing a
  line with a block comment counts). Docstrings count (they are content); blank
  and comment-only lines do not. Known heuristic bounds: comment markers inside
  string literals can fool the scanner, and a `#`-leading line inside a Python
  docstring is skipped - both conspicuous in review, neither load-bearing.
