...
## 2026-09-26 — approved visual direction connected to the real application

Owner direction in this session: preserve the LEFT reference's dense, factual three-column dashboard, blue actions, colored district tags, pale-blue calculation heading, striped tables and contextual green/pink states. Search belongs on the same page. Maps, drafting and detail tools float over the property dashboard and retain work when closed. The owner explicitly requested implementation after being told the prior hosted design was a disconnected prototype.

### Prototype versus application

The hosted `nyc-buildability-dashboard-preview.mlf332.chatgpt.site` artifact is a separate design prototype. Its image crops came from the owner's reference picture, not verified imagery of 298 Wallabout. None of those image crops, example dimensions or example calculated capacities is used by the connected implementation.

The connected implementation is an additive `/property/workspace?ruleeval=on` route in `apps/web`, linked from the existing architect navigation as **Single-page dashboard**. It shares the same server-side internal rule-evaluation gate, including `ruleeval=off` fail-closed behavior, and the same typed API clients and contracts. Existing routes remain available for regression comparison and a gated rollout. This route is NOT a new backend, public-product launch, production approval or orchestrator acceptance. The global required disclaimer is unchanged. No new dependencies, keys, authentication shortcuts, citywide downloads or frontend zoning calculations are introduced.

### Screen-to-code integration map

| Dashboard element | Real producer / reused consumer | Required meaning and behavior |
|---|---|---|
| Persistent address search | `AddressResolutionScreen` / `AddressAutocomplete` / Geoclient resolution | Suggestions remain candidates. An explicit validated Continue commits the BBL. Raw typed text, city match, representative PLUTO address and warnings retain their distinct meanings. |
| BBL alternative | Existing `validateBblInput` + `useProperty` | Same four validation outcomes; last requested property wins; a foreign returned profile cannot mount analysis/maps. |
| Header address | `recalledAddress` plus normalized profile address | Same-BBL frontage re-confirmation refreshes the display without resetting the proposal. Different-BBL selection resets property-specific work. |
| Property map | Existing MapLibre `LotOutlineMap`; validated multi-lot `ParcelStudyMap` | Actual API geometry, all supplied rings/parts; no reference image. City basemap/labels may independently fail. Identity, source dates, approximation and non-survey scope remain visible or in expandable source details. |
| Parcel identities | `useCondoRecords` + `deriveCondoSurface` + `deriveParcelStudyScope` | Billing identity is not additional land. Complete validated base identities are needed before drawing a set. No hard-coded two-parcel limit. |
| Lot/building facts | Canonical profile `FactValue` / provenance records | Values are source records, with conflict/coverage/unknown labels and source inspection. Existing buildings are expandable. Zero and unknown stay distinct. |
| District chips | Canonical zoning districts/overlays/special districts | Colors classify record categories. A chip is not an applicability determination or a zoning approval. |
| FAR and floor-area summary | Existing `residentialReference`, `evaluatedResidentialFar`, `scenarioCap`, `bulkRow` guards | Source FAR is separate from evaluated FAR. No use-FAR addition, condo area addition, allocation, geometry union or frontend arithmetic. Mismatched analysis identity withholds both numerical summaries. |
| Condo calculations | Existing monotonic condo decision | Both scenario and evaluation are suppressed for unresolved/multi-lot/error cases. Original returned documents remain accessible as withheld evidence, including inside the report. |
| Height/coverage/yards | Existing guarded bulk rows and full `DevelopmentLimits` | Unsupported or uncalculated dimensions remain honest. Rearranging the interface does not implement missing R7 rules, a combined-site envelope or measured buildable width/depth. |
| Pink review panel | Current profile conflicts, missing inputs, stale source, condo and geometry signals | Only current issues create issue rows. Full original descriptions and source records remain in Issues/Zoning/Evidence. Draft/professional-review status is always available. |
| Floating detail windows | `FloatingWorkspaceWindow` + existing facts/zoning/evidence/scenario/document/report components | Modeless, no backdrop, keyboard/pointer movement and resizing, maximize/restore, Escape and focus return. The dashboard stays present. |
| Parcel study | Existing `ParcelStudyPanel` from prerequisite PR #243 | Together/separate/compare and retained-building choices survive closing. This is planning intent, not a legal merger/subdivision or allowance allocation. Download/restore remains choices-only. |
| Proposal editor | Existing `MaxEnvelopePanel` + `ProposalEditor` / server bridge/check APIs | One retained editor per property; supported existing tools only. Unresolved condo sites cannot present combined-site proposal/envelope checks as available. No invented geometry or selected default legal site. |
| Report | Existing `ReportView` with the parent's current condo decision | Same data/withholding as screen, original analysis evidence preserved, report-only print layout plus required global disclaimer. |

### Lifecycle and honesty corrections needed by floating tools

1. A window is mounted only on its first opening, then stateful tools stay mounted while hidden. This prevents hidden-map initialization and avoids discarding an architect's draft merely because a tool is closed. Property identity is the reset boundary.
2. Both parcel-map wrappers observe container resize/reveal and call `map.resize()`. Their bounded render watchdog pauses while hidden and resumes its remaining visible-time budget. Readiness still requires real rendered features, never just a source/layer registration. A single-lot response with an absent or different BBL is rejected before rendering.
3. Proposal checks and outline conversion now carry request/revision guards and AbortSignals. Editing while a request is pending invalidates it; late success or failure cannot label newer work with old results. Converted points cannot overwrite numeric edits made after conversion began. The accepted omission count belongs to the accepted conversion response.
4. Opening a same-property legacy detail link inside a tool is routed to another floating tool. External source links retain normal browser behavior. Clicking a different valid property link deliberately updates the dashboard property and resets that property's working editor.
5. Numerical withholding never deletes returned evidence. Conditionally unusable analysis remains explicitly labeled original/withheld source material; it cannot enter the summary or acquire approval wording.

### Delivery gates and remaining work

- **Integration gate:** independent source review, remote lint/TypeScript/build, component regressions and Playwright journeys on the exact pushed commit. No local npm/npx/node execution.
- **Visual gate:** inspect desktop/tablet/phone captures; verify all reference colors and hierarchy, readable rows, real map canvas, no horizontal overflow, accessible floating controls and no draft loss. Keeping every detail expanded on a phone is not the design: facts are immediately available and details open on demand.
- **Live gate:** after orchestrator integration and production approval, walk 298 Wallabout on the deployed route; verify returned billing/base identities and actual city geometry, per-parcel failures, source freshness, unknown dimensions and combined-site withholding. CI synthetic Wallabout identities are transport/UI scaffolding and are never proof of that property's real geography or entitlements.
- **Backend packets remain separate:** reviewed legal-site authorization, combined/separate projected geometry, supported R7/other rule families, street/lot-line measurement, remaining-rights accounting, dimensioned envelope and unit yield. This UI does not claim those have been implemented.
- **Orchestrator record:** the supervising session records this owner direction, binds the PR and prerequisites, evaluates the evidence and chooses rollout. This session neither reads nor writes `project-control/` and never runs its control tool.

### Recorded implementation verification

- **Review branch:** `feature/architect-dashboard-2026-09-26`; [PR #244](https://github.com/martin10101/nyc-buildability/pull/244), stacked on prerequisite [PR #243](https://github.com/martin10101/nyc-buildability/pull/243). Neither PR is merged or deployed by this session.
- **Verified application commit:** `c388b137fc3f8e3737a12e40bda4364cab732e68`. [Remote CI run 36226522450](https://github.com/martin10101/nyc-buildability/actions/runs/36226522450) passed all **18 jobs**, including lint, TypeScript, production build, **1,680 Vitest tests in 68 files** and **139 Chromium Playwright journeys**. Context-budget and secret-scan are separate workflow checks. No local npm/npx/node was executed.
- **Connected browser coverage:** explicit address confirmation populates the dashboard without leaving its route; city-outline canvas actually reaches rendered state; proposal text survives close/reopen; floating map movement, maximize/restore and Escape work; report opens in place; desktop (1440), tablet (1024) and phone (390) layouts have no document-level horizontal overflow; the mobile proposal window stays inside the viewport. The multi-parcel journey retains Together mode and existing-building choices while continuing to withhold unsupported combined-site allowances. The explicit server kill switch returns 404 without fetching property data.
- **Independent review corrections:** both original analysis records remain inspectable when either returned identity is foreign/missing or condo/site status withholds allowances; mismatches suppress both calculated summaries. Switching tool windows restores focus to an available dashboard control rather than a hidden opener. A legacy ambiguous-parcel test now requests the BBL actually contained in its fixture; strict identity protection was retained. TypeScript's error-outcome branch is explicitly narrowed.
- **Visual evidence:** the successful run uploaded `playwright-evidence` (artifact `10900798937`, 177,564,528 bytes). The GitHub connector produced a file reference, but this session's file-transfer limit is 32 MiB, and direct retrieval returned HTTP 403; that archive was not visually inspected. An additive upload step now publishes only `test-results/**/connected-*.png` as `connected-dashboard-screenshots`, preserving the full evidence archive, pinned action, existing jobs and seven-day retention. The tests write named PNG files explicitly because buffer-only report attachments are not present under `test-results`. The PR records the subsequent screenshot inspection and exact artifact/run. Automated layout assertions are evidence of behavior, not a substitute for judging the pictures.
- **Release boundary:** this is an implemented, tested integration for orchestrator review. It is not production acceptance. The actual 298 Wallabout live walk remains a deployment gate; the synthetic Wallabout browser fixture is not evidence of its real dimensions, parcel geometry or development rights.

The assessment and implementation are ready for the orchestrator to record with their prerequisites and remaining backend/live-review packets. The PR's current checks and visual-review note are the final delivery evidence; this document does not grant publication, legal-site confirmation or rule approval.

### Screenshot-driven refinement

The named captures from commit `8ead51265ef4c9ecae943575320e3f7b0cea508c`, run `36227471783`, artifact `10901545429` were downloaded and inspected. They exposed excessive inherited search/table spacing, tablet actions below the main columns, and introductory limits cards above mobile proposal inputs. The integration now imports its scoped overrides last, uses compact source-status rows with unchanged exact values and standard ft/sq-ft unit labels, moves Quick actions above the tablet/phone grid, places mobile development limits immediately after property/map, and sizes floating maps to leave room for source/accuracy information. Preliminary development limits remain complete in an expandable section; the Envelope shortcut opens that section directly, while Draw a proposal exposes the editor first. No disclosure or source record was removed. The new browser assertions require the mobile proposal-label field to appear within the floating window and the Envelope shortcut to open its details. The PR records the final post-refinement CI and screenshot disposition.

A second visual pass on commit `1e37dee7c2c6cc57c1b49c290dd87ea9f865885b` (run `36228200199`, all 18 CI jobs passed; artifact `10902320077`) confirmed the spacing, responsive order and immediately visible mobile proposal fields. It also caught a camera-framing defect: fixed 72-pixel map padding left only 16 pixels for a parcel in a 160-pixel dashboard map. Single- and multi-parcel context-map padding now scales with the available container size; fitting after a resize uses the updated canvas dimensions. This changes camera presentation only, preserves the inspected source geometry and accuracy notices, and does not reset an architect's camera after the map is ready. Final-head CI and reviewed screenshots are recorded in PR #244.

## September 26 live release and condo-outline correction

### Release state supersedes the earlier pre-release notes

With the owner's subsequent implementation, merge and deployment authorization, PRs #243 and #244 were merged into `candidate/D-024-mrl-option-b`. Frontend commit `ac785dc19c31540928f662e6ade5f9a84e47786f` went live in Render deployment `dep-das0rd0u01pc73e7icog` at 2026-09-26 18:25:33 UTC. Its tree matches tested application commit `6edf9889492d81c6bc44bec58a7818c9bd7a9e72`: remote CI run `36228786018`, all 18 jobs passed, 1,686 component/unit tests and 139 Chromium journeys. The new dashboard is `/property/workspace?ruleeval=on`; the original landing page remains available. No backend deployment or project-control modification was part of that release.

### Observed Wallabout failure and underlying distinction

The live 298 Wallabout Street lookup resolves to condo billing BBL `3022647515`; condo records identify base BBLs `3022640032` and `3022640033`. The billing lookup had an approximate outline available during address confirmation. The two base-lot outline responses reported no outline, and their property-profile reads reported no matching property record. A source can provide a parcel identifier without providing a separately retrievable polygon for that identifier. This observation does not establish why those particular base polygons are absent or whether the records describe a legally established development site.

The initial dashboard integration switched to base-only map requests after recognizing a multi-lot condo. It omitted the separate billing-outline request and therefore discarded useful display context. Its generic “No parcel outlines available to draw” message meant the app had no usable features to render; it did not diagnose a failure of the architect's sketching gesture. Browser/WebGL limitations are a separate condition.

### Bounded correction and acceptance requirements

- Fetch the validated condo billing BBL's outline separately from the base-parcel records, with the same cancellation, retry and property-identity protections. The billing record never becomes an additional land parcel or a study selection.
- When no individual parcel geometry is usable, show a matching, usable, review-free outline explicitly classified by the source as a condo billing lot. Label it **“Condo tax-map outline · context only”**, with an unnumbered, visually distinct boundary. Preserve the original geometry, all rings, attribution, accuracy, retrieval/version information and missing-base explanations.
- Apply the correction consistently to the compact dashboard, floating property map and parcel-study map. If a base outline is available, retain that base map. Do not use a billing outline to fabricate internal parcel divisions, a union, survey dimensions, available rights or a buildable envelope.
- Distinguish unavailable source outlines, request failures, review-required geometry, loading and browser rendering failures. A refresh retries the source reads; it does not certify the property arrangement.
