...
- **Visual evidence:** the successful run uploaded `playwright-evidence` (artifact `10900798937`, 177,564,528 bytes). The GitHub connector produced a file reference, but this session's file-transfer limit is 32 MiB, and direct retrieval returned HTTP 403; that archive was not visually inspected. An additive upload step now publishes only `connected-dashboard-*/**/*.png` as `connected-dashboard-screenshots`, preserving the full evidence archive, pinned action, existing jobs and seven-day retention. The PR records the subsequent screenshot inspection and exact artifact/run. Automated layout assertions are evidence of behavior, not a substitute for judging the pictures.
