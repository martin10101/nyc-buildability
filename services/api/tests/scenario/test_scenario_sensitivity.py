...
# ---------------------------------------------------------------------------
# D-059-R003 (M5-T028): SENSITIVITY_LABEL must be DERIVED from the rule
...
def test_d059_r003_sensitivity_label_names_the_low_density_section():
    """The low-density (R5) evaluated rule -> the sensitivity label names ZR 23-21
    (hand-typed from r5_residential_far.rule.json's own ``section`` citation)."""
    document = _preliminary_document()
    result = analyze_scenario_sensitivity(document, VAR, [0.5, 0.8])
    assert "under ZR 23-21" in result["label"]
    assert "23-22" not in result["label"]


def test_d059_r003_sensitivity_label_names_the_r6_r12_section():
    """An R6-R12 evaluated rule -> the sensitivity label names ZR 23-22 (hand-typed from
