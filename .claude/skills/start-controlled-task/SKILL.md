...
3. **A task packet CANNOT be claimed unless it explicitly names all of the following.** If any is missing, complete the packet first — do not claim:
   ...
   **Modularity boundary answers (permanent, M0-T073 — `docs/CODE_MODULARITY_POLICY.md`).** A packet whose scope touches handwritten production source must answer, in the packet or its plan: (1) which responsibility owns this change; (2) which existing module should contain it; (3) whether the target file is already above a warning threshold (`python tools/modularity_check.py --report`); (4) whether behavior should be extracted before adding more; (5) what public interface must remain stable; (6) which focused tests protect the boundary; (7) that the modularity CI check passes before submission. Independent code review checks these answers against the actual diff.
