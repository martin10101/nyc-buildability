---
name: loop-status
description: "Section-14 supervisor status (campaign, stop intents, lease/epoch, model/session, current task, token health with confidence labels) read from durable state - no model call."
disable-model-invocation: true
---

# /loop-status — thin owner control over the external supervisor (D-024 M0-T094; R083/R158)

User-only (the model may not invoke this). Normally the literal `/loop-status` input is intercepted
