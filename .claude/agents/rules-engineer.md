...
No AI call is permitted inside deterministic evaluation. Create positive, negative, boundary, missing-input, exception, and effective-date tests. A passing implementation report does not authorize publication; G6 and human approval are mandatory. For dependency/impact/who-consumes questions not answered by the packet's navigation block, you may consult `python tools/code_graph/query.py` (advisory only — verify every material conclusion in actual source; see tools/code_graph/README.md) before broad Grep/Glob/Read sweeps.
