# D-007 — source-001 (verbatim owner directive) — Codex ↔ Claude Code Supervisor Bridge (v4.3, Autonomy-Tuned)

Captured verbatim per `.claude/skills/directive-compliance` §1. Channel: owner_document — the
owner authored and revised this build directive (v1 → v4.3), tracked it at
`.claude/CODEX_CLAUDE_SUPERVISOR_BUILD_DIRECTIVE_v4.3.md` (owner commit 148d13b, merged PR #141,
with an LF `.gitattributes` pin protecting the digest anchor), and ordered its Phase 0 and
canonical capture through D-004 amendment 17 STEP 6 (D-004-R685–R694), resumed by amendment 18
(D-004-R713/R714/R716/R717). Phase 0's read-only reconciliation preceded this capture; this file
is the FIRST of Phase 0's two permitted writes (D-004-R693; directive Section 17/21).

Head at capture time: branched from `origin/main` = `b5589d00eaeb09fc2f8cb637ea6fce9c2e0963ef`.

Source bytes verified immediately before capture: the tracked file hashes to
`426da3bb22714a403553b013e8969c6bfa424ee01d99e10d1269d0b65e0f5137`
— EXACTLY the anchor recorded in D-004-R685 and the captured amendment-17 text. The capture body
below is a byte-faithful copy of that tracked file (mechanically concatenated, not retyped);
per Section 0 of the directive itself, the tracked `.claude/` copy is the working orientation
copy and THIS capture is the canonical directive record. The v4.3 file remains tracked at its
`.claude/` path as owner-placed orientation; it is not a competing canonical source.

Supersession per the directive's own header: v1 and the uncaptured v2/v3/v4/v4.1/v4.2 drafts are
superseded by this capture.

R024 scan before commit: no session identifier, machine username, absolute user path, or
hostname. No redaction required.

---

## THE OWNER DIRECTIVE (verbatim, complete — v4.3 bytes, digest 426da3bb…)
