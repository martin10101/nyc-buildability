...
| `tools/test_mcp_policy.py` | new (37 tests) | Removal/weakening regressions for the validator incl. the G3 bypass fixtures and re-review probes 59-63 (R029) |
...
- `python tools/test_mcp_policy.py` → 37/37 OK (after the G4 MAJOR-1/MAJOR-2 corrections added the identifier-pin and CI-wiring tests)
...
- **G5 F-2 (minor):** validator/test counts in this report refreshed to the shipped
  310-line validator and (then) 35-test suite; the PR body was refreshed the same
  way. The suite is 37 tests after the subsequent G4 MAJOR-1/MAJOR-2 corrections
  (per the DCV staleness note, counts above re-refreshed accordingly).
