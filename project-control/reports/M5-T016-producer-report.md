...
| `AddressResolutionScreen.tsx` (slimmed) | Keeps ONLY the state machine + loading card; `resolved`/`resolved_with_warnings` now route to the confirm card; new `notMyProperty` (clear result, RETAIN form values, focus street input). 306 raw lines — back under every modularity threshold. |
...
| S5 disclosure | confirm S5 tests 1–3 | open-by-default; full endpoint URL leaked; ids conflated; params/facts unrendered; a "Verified" badge; withheld reason hidden |
