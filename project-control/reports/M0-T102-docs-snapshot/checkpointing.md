# Snapshot: Claude Code docs — Checkpointing

- Source URL: https://code.claude.com/docs/en/checkpointing
...
* Claude Code deletes checkpoints along with sessions after 30 days, following the retention sweep rules; change the period with `cleanupPeriodDays`
...
> **Note:** Summarize keeps you in the same session and compresses context, like a targeted `/compact`. To branch off and try a different approach while preserving the original session intact, use `/branch` or `claude --continue --fork-session` instead.
...
A subagent makes edits with Claude's file editing tools, but Claude Code usually doesn't capture those edits in your session's checkpoints. Whether rewinding restores them depends on how the subagent runs:

* **Foreground forked skill**: a skill with `context: fork` that runs in the foreground edits your working tree during your own turn, so rewinding restores its edits as usual. Set `background: false` to run a fork in the foreground; a few situations, listed on the skills page, run it there regardless of the setting.
* **Any other subagent**: rewinding doesn't restore the edits. Use git to revert them. This includes a forked skill that runs in the background, the default, and a background `/code-review --fix` run.
...
To see which paths a restore skips, turn on debug logging with `/debug` before you restore: the debug log at `~/.claude/debug/<session-id>.txt` names each skipped path. For every skip reason and the recovery steps, see the skipped-files entry in the error reference.
...
* Requires Claude Code v2.1.191 or later: the rewind menu's `/resume <session-id> (previous session)` entry after a `/clear` in the same process. On earlier versions, run `/resume` and pick the previous session from the list instead.

## Windows caveats (as stated on the page)

* The page states no Windows-specific caveats. (The symlink/hard-link restore skip applies on all platforms; the page names dotfile-manager symlinks and pnpm hard-links as examples.)