# Snapshot: Claude Code docs — Cross-session messaging

- Source URL: https://code.claude.com/docs/en/cross-session-messaging
- Fetch date: 2026-08-26
- Purpose: D-024 amendment-3 capability re-baseline, requirement D-024-R147
- Capture method: WebFetch of the official page; content reproduced faithfully from the fetched markdown (link targets flattened to text where they pointed at other doc pages). Snapshot only — no editorializing.
...
* The reply target on this machine fails a safety check, such as a symlinked target or an endpoint that isn't the expected process. The errors reference ("Refusing to send a cross-session message") lists these checks.
* Claude addresses the message to this session's own name.
...
Each side's inbound controls apply to a notice like a message:
...
* **Teammates**: this session's own agent team teammates. Before v2.1.239, teammates didn't appear in the listing, though Claude could already message them by name.
...
* **Your cloud sessions**: your Claude Code on the web sessions, shown while this session is connected to Remote Control. Claude Code labels them `cloud` in the listing.
...
Claude addresses a session beyond this machine by name, the same as a local session.

A session answers to the name you set with the `/rename` command or the `--name` flag. When you don't set one, Claude Code names the session itself. For an interactive session, that is the name shown in listings of running sessions.
...
If this session isn't connected to Remote Control when Claude sends to a session beyond this machine, the message still goes through, but without a reply address, so the receiving Claude can't answer it. Claude is told as much when it sends.
...
* Claude Code exports it to hooks and Bash commands as the `CLAUDE_CODE_MESSAGING_SOCKET` environment variable:
...
* **Own-child messages**: when no `crossSessionInbound` value applies, Claude Code delivers a message it verifies came from the session's own child processes, such as a hook or Bash command posting back to its own session's socket.
  ...
  * On macOS after the posting process has exited and in containers where Claude Code runs as process ID 1, that process evidence is missing, and Claude Code instead verifies a child that sent the session's exported `CLAUDE_CODE_MESSAGING_TOKEN` in the auth line that opened its connection. On native Windows, that token is the only way Claude Code verifies an own-child message.
...
* Background agents (agent view): dispatch and monitor the parallel sessions you might message
* Remote Control: connect this session to reach your sessions on other machines
* Settings reference: `crossSessionInbound`, `isolatePeerMachines`, and `dialogExpiry`
...
* Requires Claude Code v2.1.224 or later on macOS and Linux (including Linux inside WSL 2); v2.1.234 or later on native Windows. When a session meets the requirements, messaging is on with nothing to enable.
* Requires v2.1.232 or later: @-mention typeahead for naming a target session in a prompt; also the `/config` row "Messages from your other sessions".
* Requires v2.1.236 or later (in both sessions): idle-notice subscriptions (`notify_when_idle`).
* Before v2.1.239: teammates didn't appear in the `/list-agents` listing (though messageable by name); the listing didn't show this session's own name, and a message sent to it was reported as an agent that couldn't be found.
* Requires v2.1.225 or later: starting a conversation with a session on another of your machines (before that, Claude could only reply to a message that arrived from one). Also before v2.1.225: no expiry deadline applied to held messages in a `-p` session, and a session that ended with held messages reported nothing to their senders.
* Before v2.1.236: bursts past a session's inbox capacity were reported as sent while the receiving session dropped them.

## Delivery and durability guarantees (as stated on the page)

* Delivery timing: the receiving Claude reads the message between tool calls during an active turn (a running tool is never interrupted); an idle session starts a new turn with the message.
* Delivery is NOT guaranteed. Messages can be refused at the sender (size cap of about a million serialized characters same-machine, inbox burst cap, safety-check failure on the reply target, self-addressing), and at the receiver each message is Delivered, Held, or Refused per inbound controls. Held messages expire per `dialogExpiry` (default five minutes) and are dropped; the held store caps at 100 (oldest dropped); the accepted-message queue caps at 50; per-sender rate limits and identical-repeat drops apply.
* Durability: no durable store beyond the mechanisms above — a message dropped, refused, or expired is gone (senders that can be reached are notified); a delivered message appears in the receiving conversation and stays there. A one-way cross-machine message (sender not connected to Remote Control) carries no reply address, so the receiving Claude can't answer it.

## Security/identity model (as stated on the page)

* Same-machine transport: per-session Unix domain socket (macOS/Linux/WSL 2) or per-session named pipe (native Windows); same-machine messages never pass through Anthropic servers. Cross-machine and cloud messages travel through Anthropic servers (over Remote Control for other machines).
* The socket is restricted to your operating-system user on macOS/Linux; on native Windows each connection must authenticate first with a key only your operating-system user can read. On a shared machine another user's sessions can't deliver to it.
* Per-session token `CLAUDE_CODE_MESSAGING_TOKEN`; auth line optional on macOS/Linux/WSL 2, required on native Windows; own-child verification is by process evidence (Linux) or token (macOS-after-exit, PID-1 containers, native Windows).
* An incoming message can't approve anything, can't change configuration, its embedded slash commands never run, and the receiving session's own permission prompts still fire. A message never carries conversation history or files. Permission boundaries stay per-session.
* `isolatePeerMachines: true` requires explicit approval before any message leaves the machine, even in `bypassPermissions` mode; a `true` from any settings scope applies (a checked-in project file can turn it on but not off).

## Windows caveats (as stated on the page)

* Native Windows requires Claude Code v2.1.234 or later (vs v2.1.224 on macOS/Linux/WSL 2).
* Transport on native Windows is a per-session named pipe, not a Unix domain socket.
* On native Windows the auth line is required on every socket connection, and the token is the only way Claude Code verifies an own-child message (no process evidence exists there).
* A session inside WSL 2 and a native Windows session on the same computer can't reach each other: they register under different home directories and listen on different socket types.