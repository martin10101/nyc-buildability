# Snapshot: Claude Code docs — Worktrees

- Source URL: https://code.claude.com/docs/en/worktrees
...
> **Note:** Worktrees require a git repository; for other version control systems, configure hooks to replace the git logic. In the desktop app, every new session gets its own worktree automatically.
...
Most sessions need only the first two sections: start Claude in a worktree, then clean up when you exit. Return to the rest of the page when you need to resume a session, change how worktrees are created, or debug a failure.
...
You can also ask Claude to "work in a worktree" during a session, and it creates one with the `EnterWorktree` tool. Once in a worktree, Claude can switch directly to another one under `.claude/worktrees/` by calling `EnterWorktree` with the target path; the previous worktree stays on disk untouched.
...
> **Note — Hook paths don't follow the worktree.** After Claude enters a worktree, Claude Code keeps `${CLAUDE_PROJECT_DIR}` in your hooks where it was and passes the worktree path to them a different way:
>
...
> * **`cwd` follows Claude**: the `cwd` field in the hook's input JSON is the worktree root, and it moves again when Claude runs `cd`. Read it when a hook needs the worktree path.
...
> **Note:** Before v2.1.212, a non-interactive resume stayed in the starting directory and `ExitWorktree` reported that there was no active worktree session to exit.
...
The same enforcement covers every subagent Claude spawns from the isolated session. It applies whether the session is interactive or runs in the background. Subagents that run in their own worktree carry the same checks. Their version history is under "Write subagent files" on the sub-agents page.
...
Subagents can run in their own worktrees so parallel edits don't conflict. Ask Claude to "use worktrees for your agents", or make the isolation permanent for a custom subagent by adding `isolation: worktree` to its frontmatter.
...
Each subagent gets a temporary worktree that Claude Code removes automatically when the subagent finishes without changes; a worktree with changes stays on disk until the periodic sweep (below) can remove it without losing work.
...
Configure a `WorktreeCreate` hook to replace the default `git worktree` logic entirely, including placing worktrees somewhere other than `.claude/worktrees/`. For a complete example, see "Non-git version control".
...
* **The repository's `.git` directory**: git commands in a worktree write to the main repository's shared `.git` directory, and sandboxing allows those writes, so commands such as `git commit` work from inside a worktree with the sandbox enabled.
* **Plugins**: plugins installed at project scope from the main checkout also load in worktrees of the same repository, so you don't need to reinstall them per worktree. Requires Claude Code v2.1.200 or later.
* **Permission approvals**: choosing "Yes, and don't ask again" for a Bash command in a worktree session saves the rule to the main checkout's `.claude/settings.local.json`, so it applies in the main checkout and in every other worktree of the repository, and it survives the worktree's removal. On Windows and the other cases where Claude Code keeps the local file in the starting directory, the rule stays with that worktree. Before v2.1.211, an approval granted in a worktree was saved inside that worktree, didn't apply elsewhere, and was lost when the worktree was removed.

All three apply whether you create the worktree with `--worktree`, with `git worktree add`, or through the desktop app.
...
See the Git worktree documentation for the full command reference.
...
When Claude Code can't enter the worktree directory at startup, it prints an error naming the path and exits with code 1. This can happen when a `WorktreeCreate` hook prints something other than the directory it created, or when the directory was deleted after it was set up.
...
Claude Code leaves the refused directory in place, since it may hold work. Match the message to its recovery, whether it follows `Refusing to use <path>` or appears in a resume message; some endings occur only in resume messages:
...
In non-interactive mode with `-p`, and on resumes the Agent SDK runs, Claude Code stops the resume with a stderr error for every refusal except a gone worktree, instead of continuing without isolation, and the messages take different shapes from the ones in the table above:
...
* Before v2.1.206: Claude could enter any existing worktree path without asking (the EnterWorktree approval prompt for paths outside `.claude/worktrees/` postdates this).
* Before v2.1.205: removing a worktree with a link nested in a subdirectory could delete the folder it pointed to (Windows link handling).
* Before v2.1.212: a non-interactive resume stayed in the starting directory and `ExitWorktree` reported no active worktree session to exit; separately, worktree creation followed a committed symlink at `.claude`, `.claude/worktrees`, or the worktree directory and could create files outside the repository.
* Requires Claude Code v2.1.198 or later: the transcript follows the session into/out of a git-created worktree (recorded under the new working directory).
* Before v2.1.210: a lock left by a killed session stayed in place until a manual `git worktree unlock` (the sweep's stale-lock release postdates this).
* Before v2.1.208: a fresh worktree used whatever `origin/HEAD` was already cached locally; and reusing a worktree name always reopened the old worktree at its old tip.
* Before v2.1.233: `--worktree` accepted only `#<number>` and GitHub-style pull request URLs, and always fetched `pull/<number>/head`.
* Before v2.1.239: a `**/` pattern in `.worktreeinclude` copied files out of a wholly ignored directory only when the directory itself matched the pattern.
* Requires Claude Code v2.1.200 or later: project-scope plugins installed from the main checkout also load in worktrees.
* Before v2.1.211: a permission approval granted in a worktree was saved inside that worktree, didn't apply elsewhere, and was lost when the worktree was removed.

## Windows caveats (as stated on the page)

* On Windows, removing a worktree doesn't delete files outside it: an NTFS junction or directory symlink inside the worktree is deleted as a link only, keeping the folder it points to (before v2.1.205 a nested link's target could be deleted).
* On Windows, and the other cases where Claude Code keeps the local settings file in the starting directory, a permission approval granted in a worktree stays with that worktree instead of being saved to the main checkout's `.claude/settings.local.json`.
* For PowerShell commands, worktree isolation applies only the working-directory check (not the git-redirect or command-shape checks).