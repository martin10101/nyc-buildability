# Snapshot: Claude Code docs — Remote Control
...
- Capture method: WebFetch of the official page; content reproduced faithfully from the fetched markdown (link targets flattened to text where they pointed at other doc pages; Tabs/Steps components flattened to headings/lists). Snapshot only — no editorializing.
...
> **Note:** Remote Control is available on all plans. On Team and Enterprise, it is off by default until an Owner enables the Remote Control toggle in Claude Code admin settings.
...
* **Work from both surfaces at once**: the conversation and the progress of subagents and dynamic workflows stay in sync across all connected devices, so you can send messages from your terminal, browser, and phone interchangeably.
...
| `-c`, `--continue` | Bring back the session that the last server in this directory started with, instead of creating a new one. Can't be combined with `--session-id`, `--spawn`, `--capacity`, or `--create-session-in-dir`. Requires Claude Code v2.1.200 or later; earlier versions reject the flag as an unknown argument. |
| `--session-id <id>` | Bring back one session by its ID. Can't be combined with `--continue`, `--spawn`, `--capacity`, or `--create-session-in-dir`. Requires Claude Code v2.1.200 or later; earlier versions reject the flag as an unknown argument. |
...
In the Claude Code VS Code extension, type `/remote-control` or `/rc` in the prompt box, or open the command menu with `/` and select it.

A banner appears above the prompt box showing connection status. Once connected, click **claude.ai/code** in the banner to go directly to the session, or find it in the session list at claude.ai/code. Claude Code also posts the session URL in the conversation.
...
* **Repeated permission prompts**: after you answer several permission prompts in a session, an **Approve tool calls from your phone** notification shows the session URL. Claude Code removes it when your next turn starts.
...
If you didn't set an explicit name, Claude Code updates the title to reflect your prompt once you send one. Claude Code matches auto-generated titles to the language of your conversation, or to the `language` setting if one is configured; the language matching requires Claude Code v2.1.176 or later.

When you rename a session from claude.ai or the Claude app, Claude Code also updates the local title shown in `claude --resume`. Claude Code applies the same rename to the session name shown on the prompt bar, and in the `claude agents` listing when the session runs in the background. Before v2.1.221, renaming from the session list at claude.ai or in the Claude app updated only the title, and the CLI kept its previous session name; `/rename`, which runs in the CLI itself, set the name on any version.
...
* **Compaction and `/clear`**: while Claude Code compacts the conversation, connected devices show the progress and then where the conversation was compacted. When you run `/clear`, the conversation resets on connected devices too.
...
* **Messages from your other sessions**: with cross-session messaging, the same connection carries messages between your own sessions on different machines and from your Claude Code on the web sessions, through Anthropic servers like the rest of Remote Control traffic. "Message sessions on other machines" covers the delivery rules and "Control inbound messages" covers the inbound controls. Requires Claude Code v2.1.224 or later.
...
* **`false`**: turn auto-connect off, though a `true` from managed settings outranks it, because Claude Code saves the choice to your user settings. A `false` in project or local settings (`.claude/settings.json`, `.claude/settings.local.json`) turns auto-connect off even over a managed `true`.
...
With this setting on, each interactive Claude Code process registers one remote session. If you run multiple instances, each one gets its own remote session. To run multiple concurrent sessions from a single process, use server mode instead.
...
To bring back a session you started with `claude --remote-control` or `/remote-control`, resume the conversation with `claude --continue` or `claude --resume`; whether Claude Code reconnects, and to which session, depends on the conversation's reconnection record. If you resume the conversation in a second terminal while the first one still has Remote Control on, Claude Code prints a notice in the second terminal and leaves Remote Control off there instead of taking the session away from the first. Run `/remote-control` in the second terminal to move Remote Control to it.
...
To turn Remote Control off entirely, use the `disableRemoteControl` setting. Organizations with compliance requirements such as Zero Data Retention can't enable Remote Control.

## Trusted Devices

> **Note:** Trusted Devices is currently in beta. Features and functionality may evolve as the experience is refined. Trusted Devices is available on Team and Enterprise plans. It is off by default until an Owner enables it.
...
1. **Open Claude Code admin settings**: go to the Claude Code admin settings page at claude.ai. The **Require trusted devices** toggle appears under the Remote Control setting.
...
Remote Control and Claude Code on the web both use the claude.ai/code interface. The key difference is where the session runs: Remote Control executes on your machine, so your local MCP servers, tools, and project configuration stay available. Claude Code on the web executes in the cloud.
...
* **Forwarded dialogs expire**: Claude Code keeps permission prompts and `AskUserQuestion` questions open until you answer them. When Claude Code forwards another kind of dialog to the remote session, such as the model-choice prompt shown after a safety refusal, it waits five minutes by default, then closes the dialog and continues with the dialog's no-action default. The mid-session Fable 5 usage-credits consent prompt follows the same deadline but isn't forwarded: Claude Code shows it only in the terminal where the session runs, and if nobody has answered there by the deadline, it ends the turn without sending the request. Your model selection is unchanged, and Claude Code asks again on your next message. Set `dialogExpiry` to adjust or disable the deadline. Requires Claude Code v2.1.224 or later. Claude Code applies the same deadline to the approval dialog for a held cross-session message. The held-message expiry rules cover the cases where Claude Code keeps the dialog open past it.
* **Some commands are local-only**: commands that only run in the terminal interface, such as `/plugin` or `/resume`, work only from the local CLI, whether or not you pass an argument. The following work from mobile and web:
  ...
  * `/mcp`, from v2.1.166: from the mobile app, returns a text summary of server status instead of opening the picker. On the web, `/mcp` on its own opens a directory of claude.ai connectors instead of returning the summary. The `reconnect`, `enable`, and `disable` subcommands work from both. Unlike the local CLI, `/mcp reconnect` without a server name reconnects every server that has failed or needs authentication.
  ...
  * On Team and Enterprise, `/usage-credits` from mobile or web doesn't send a usage-credits request to your admin. Sending requires a confirmation that appears only in the interactive CLI, so the command tells you to run it there instead. Before v2.1.211, the text form sent the request without confirmation.
...
One of these variables is set: `DISABLE_TELEMETRY`, `DO_NOT_TRACK`, `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`, or `DISABLE_GROWTHBOOK`. Each of them disables the feature-flag evaluation that Remote Control availability depends on, and the full message names the variable Claude Code found. Unset that variable wherever it's set, in your shell environment or in the `env` block of a `settings.json` file. On versions before 2.1.154, the same configuration produces "Remote Control is not yet enabled for your account" instead.

### "Remote Control is only available when using Claude via api.anthropic.com"

The session isn't talking to the Anthropic API directly, so there is no claude.ai backend to pair with. This happens on Amazon Bedrock, Google Cloud's Agent Platform, and Microsoft Foundry. It also happens when `ANTHROPIC_BASE_URL` points at a host other than `api.anthropic.com`, such as an LLM gateway or proxy, even if you sign in with claude.ai. Before v2.1.196, Claude Code didn't show this message for a custom `ANTHROPIC_BASE_URL`. See the error reference for the full cause list.

The message names what routed the session away from the Anthropic API, such as `CLAUDE_CODE_USE_BEDROCK` or a custom `ANTHROPIC_BASE_URL`. If you have an eligible claude.ai login, unset the named variable, remove it from the `env` key in settings if you set it there, and restart the session. Before v2.1.219, the message was only the sentence in this section's header, so on older versions check your environment yourself for provider variables such as `CLAUDE_CODE_USE_BEDROCK` and `CLAUDE_CODE_USE_VERTEX`, and for `ANTHROPIC_BASE_URL`.

### "Remote Control is disabled by your organization's policy"

A policy blocks Remote Control. The message's own text tells you which:

* **The error mentions `disableRemoteControl`**: your IT administrator has disabled Remote Control on this device through managed settings, independent of the organization-wide toggle and of how you're signed in.
* **Otherwise, an Owner hasn't enabled it for your organization**: this form appears when you're signed in with an eligible claude.ai account but Remote Control is off, the default on Team and Enterprise plans. An Owner can enable it in Claude Code admin settings by turning on the **Remote Control** toggle. This toggle is a server-side organization setting.
...
Claude Code couldn't bring back the previous Remote Control session and stopped instead of starting a new one on its own. You can see this message after you resume a conversation with `claude --resume` or `claude --continue`, or after Claude Code reconnects on its own following a disconnect.
...
| Self-hosted environments | Start a cloud session and pick your organization's environment | Your organization's infrastructure | Deploy runners, on Team and Enterprise plans | Cloud sessions that must run inside your network |
| Scheduled tasks | Set a schedule | CLI, Desktop, or cloud | Pick a frequency | Recurring automation like daily reviews |

## Related resources

* Claude Code on the web: run sessions in the cloud instead of your machine, configured through cloud environments
* Cross-session messaging: let Claude message your sessions on other machines or on Claude Code on the web
...
## Version-gate index (every version note stated on the page)

* Before v2.1.196: Claude Code allowed Remote Control with a custom `ANTHROPIC_BASE_URL` (now blocked), and didn't show the "only available via api.anthropic.com" message for that configuration.
* Requires v2.1.200 or later: `claude remote-control --continue` and `--session-id` flags (earlier versions reject them as unknown arguments). Before v2.1.200: Claude Code created a new session after any reconnection failure.
* Requires v2.1.176 or later: auto-generated title language matching.
* Before v2.1.221: renaming from claude.ai or the Claude app updated only the title, and the CLI kept its previous session name (`/rename` in the CLI set the name on any version).
* Requires v2.1.224 or later: cross-session messages over the Remote Control connection; forwarded-dialog `dialogExpiry` deadline; stale-token self-refresh on Remote Control startup (before v2.1.224 a stale token failed startup, so auto-connect sessions could fail intermittently at launch).
* Requires v2.1.234 or later: picking an effort level from a connected device's effort control.
* Requires v2.1.203 or later: the VS Code "Enable Remote Control for all sessions" toggle.
* Requires v2.1.228 or later: `--continue` and `--session-id` unarchive an archived session.
* From v2.1.166: `/mcp` from mobile/web. From v2.1.181: `/config` from mobile/web; also `CLAUDE_CLIENT_PRESENCE_FILE` (as of v2.1.181). Before v2.1.211: the text form of `/usage-credits` sent the admin request without confirmation. From v2.1.221: `/autocompact` from mobile/web.
* Before v2.1.206: `/remote-control` while signed out reported `Unknown command: /remote-control`.
* Before v2.1.239: the account-check error read "Remote Control is not yet enabled for your account". Before v2.1.154: feature-flag-disabling variables produced that same message instead of "Remote Control requires feature-flag evaluation".
* Added in v2.1.178: "Couldn't verify Remote Control eligibility" and "Couldn't verify your organization's Remote Control policy".
* Before v2.1.219: the api.anthropic.com error message didn't name what routed the session away from the Anthropic API.
* Reconnection-record history: before v2.1.232, different replacement behavior and a different message text; v2.1.227 through v2.1.231 refused replacement even on account match; through v2.1.226 replaced regardless of account match; v2.1.224 through v2.1.226 created the replacement under the machine's signed-in account without uploading earlier messages; before v2.1.227 the record didn't name the owning account.
* Added in v2.1.225: "Remote Control got an unexpected server response".

## Security / transport / MCP summary (as stated on the page)

* Transport: the local session makes outbound HTTPS requests only and never opens inbound ports; it registers with the Anthropic API and polls for work; connected devices are routed over a streaming connection through the Anthropic API over TLS. The connection uses multiple short-lived credentials, each scoped to a single purpose and expiring independently.
* Data: while connected, the session transcript (messages, responses, tool activity) is stored on Anthropic servers for cross-device sync and reconnection; execution and filesystem access stay on the machine; stored transcripts are retained under the Data usage policy. Organizations with compliance requirements such as Zero Data Retention can't enable Remote Control. The `disableRemoteControl` setting turns Remote Control off entirely.
* MCP implications: Remote Control sessions run on your machine, so your local MCP servers, tools, and project configuration stay available remotely (unlike Claude Code on the web, which executes in the cloud). `/mcp` works from mobile (text summary of server status) and web (connector directory), with `reconnect`, `enable`, and `disable` subcommands from both; `/mcp reconnect` without a server name reconnects every failed/needs-auth server.
* Authentication: claude.ai sign-in required (Pro, Max, Team, Enterprise; API keys and long-lived tokens from `claude setup-token` / `CLAUDE_CODE_OAUTH_TOKEN` are not supported); on Team/Enterprise an Owner must enable the org toggle; Trusted Devices (beta, Team/Enterprise) adds per-device enrollment plus a sign-in no older than 18 hours with biometric/passkey step-up; Anthropic never receives or stores biometric data (only the device's public key and basic metadata).
* What an operator can do remotely: send prompts (queued mid-turn), see and stop running subagents/workflows, approve tool calls/permission prompts, attach images and files, pick model and effort level, rename sessions, and run the listed text-output and argument-form commands; local-only terminal commands (such as `/plugin`, `/resume`) do not work remotely.

## Windows caveats (as stated on the page)

* Windows Hello is one of the accepted Trusted Devices presence/step-up mechanisms (alongside Face ID, Touch ID, and passkeys).
* The page states no other Windows-specific caveats for Remote Control itself.