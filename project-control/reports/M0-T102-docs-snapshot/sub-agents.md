# Snapshot: Claude Code docs — Subagents
...
- Capture method: WebFetch of the official page; content below is the complete page markdown, verbatim as served. No editorial changes.
...
### Step 1: Ask Claude to create the subagent
...
Keep `name` values unique across the whole tree: if two files under the same `.claude/agents/` directory, including its subfolders, declare the same name, Claude Code loads only one of them, chosen by filesystem read order rather than a documented precedence. Across nested project directories, the definition closest to the working directory wins, as described above. The [`/doctor`](/docs/en/commands#all-commands) setup checkup reports files in the same directory that share a name and proposes renaming or removing all but one.
...
---
name: data-scientist
description: Data analysis and visualization specialist. Use for exploratory analysis, statistical insights, and chart generation.
tools: Read, Bash, Grep, Write
model: sonnet
---

You are a data scientist specializing in exploratory analysis and visualization.

When analyzing data:
1. Load and inspect the dataset
2. Calculate key statistics and distributions
...
Use Python with pandas, matplotlib, and seaborn for analysis and visualization.
```